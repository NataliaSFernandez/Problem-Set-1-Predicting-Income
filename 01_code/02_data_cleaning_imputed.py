...
Script 02: Data Cleaning (Imputed Version)
==============================================================================
OBJETIVO: Limpiar datos GEIH 2018 Bogotá - IMPUTA ingresos faltantes

INPUTS:  00_data/raw/geih_2018_completo.csv (16,089 obs, 178 vars)
OUTPUTS: 
  - 00_data/cleaned/data_cleaned_imputed.csv (datos limpios)
  - 02_output/figures/missing_heatmap_imputed.png
  - 02_output/figures/outliers_income_boxplot_imputed.png
  - 02_output/tables/cleaning_summary_imputed.tex
==============================================================================
"""
...
print("="*80)
print("LIMPIEZA DE DATOS GEIH 2018 - PROBLEM SET 1 (IMPUTED VERSION)")
print("="*80)
...
plt.tight_layout()
plt.savefig("02_output/figures/missing_heatmap_imputed.png", dpi=300, bbox_inches='tight')
plt.close()
print("  Guardado: 02_output/figures/missing_heatmap_imputed.png")

# ==============================================================================
# PASO 3: FILTROS DE OBSERVACIONES (SIN FILTRAR POR INGRESO)
# ==============================================================================
...
print("\nCriterios (versión imputada):")
print("  - Edad >= 18 años")
print("  - Empleados (ocu == 1)")
print("  - NO se elimina por ingreso missing (se imputará)")
...
# NOTA: NO filtramos por y_total_m > 0 en esta versión
print(f"\nNOTA: Manteniendo {data_filtered['y_total_m'].isna().sum()} obs con ingreso missing")
print(f"      Estas observaciones serán imputadas en PASO 5")
...
# ==============================================================================
# PASO 5: IMPUTACIÓN DE MISSING VALUES (INCLUYE INGRESO)
# ==============================================================================
...
# DECISIÓN 2: IMPUTAR INGRESO (NUEVO EN VERSIÓN IMPUTED)
print(f"\n2. Ingreso (y_total_m): Imputación por grupos")
missing_income_mask = data_filtered['y_total_m'].isna()
n_missing_income = missing_income_mask.sum()

if n_missing_income > 0:
    print(f"   Observaciones con ingreso missing: {n_missing_income}")
    print(f"   Método: Mediana por sexo × educación")
    print(f"   Justificación (Cuaderno 1): Preservar heterogeneidad de grupos")
    
    # Calcular medianas por grupo
    income_medians = data_filtered[data_filtered['y_total_m'].notna()].groupby(['sex', 'maxEducLevel'])['y_total_m'].median()
    
    # Imputar por grupo
    for (sex_val, edu_val), median_val in income_medians.items():
        mask = (missing_income_mask) & (data_filtered['sex']==sex_val) & (data_filtered['maxEducLevel']==edu_val)
        n_imputed = mask.sum()
        if n_imputed > 0:
            data_filtered.loc[mask, 'y_total_m'] = median_val
            sex_label = "Mujer" if sex_val == 0 else "Hombre"
            print(f"     Grupo (sex={sex_label}, edu={edu_val}): {n_imputed} obs → ${median_val:,.0f}")
    
    # Crear flag de imputación
    data_filtered['income_imputed'] = missing_income_mask.astype(int)
    print(f"\n   Flag creado: income_imputed (1={n_missing_income} imputados, 0={len(data_filtered)-n_missing_income} observados)")
else:
    print(f"\n2. Ingreso (y_total_m): 0 missings, no requiere imputación")
    data_filtered['income_imputed'] = 0

# DECISIÓN 3: Imputar horas trabajadas con mediana por sexo
if 'totalHoursWorked' in data_filtered.columns:
    ...
    if missing_hours > 0:
        ...
        print(f"\n3. Horas trabajadas: Imputadas {missing_hours} obs")
        print(f"   Método: Mediana por sexo")
        print(f"   - Mujeres: {median_female:.1f} horas")
        print(f"   - Hombres: {median_male:.1f} horas")
    else:
        print(f"\n3. Horas trabajadas: 0 missings, no requiere imputación")

# DECISIÓN 4: Imputar variables categóricas con moda
...
for var in categorical_vars:
    if var in data_filtered.columns:
        ...
        if missing_count > 0:
            ...
            print(f"\n4. {var}: Imputadas {missing_count} obs con moda = {moda}")
            print(f"   Método: Moda (valor más frecuente)")

# DECISIÓN 5: Variables numéricas continuas (tenure) - mediana
if 'p6426' in data_filtered.columns:
    ...
    if missing_count > 0:
        ...
        print(f"\n5. Tenure (p6426): Imputadas {missing_count} obs con mediana = {median_tenure:.1f}")
...
plt.tight_layout()
plt.savefig("02_output/figures/outliers_income_boxplot_imputed.png", dpi=300, bbox_inches='tight')
plt.close()
print("  Guardado: 02_output/figures/outliers_income_boxplot_imputed.png")
...
print(f"\nVariables derivadas creadas (incluye flag income_imputed)")
...
validations = {
    ...
    'Flag income_imputed existe': 'income_imputed' in data_filtered.columns,
...
print("\nEstadísticas por género (versión imputada):")
...
n_imputed = data_filtered['income_imputed'].sum()
print(f"\nEstadísticas de imputación:")
print(f"  Observaciones imputadas: {n_imputed} ({n_imputed/len(data_filtered)*100:.1f}%)")
...
output_path = "00_data/cleaned/data_cleaned_imputed.csv"
...
summary_df = pd.DataFrame({
    ...
    'Valor': [
        ...
        f"{n_imputed} ({n_imputed/len(data_filtered)*100:.1f}%)",
        "log_income, age_squared, female, is_outlier_income, income_imputed"
    ]
})
...
with open("02_output/tables/cleaning_summary_imputed.tex", "w") as f:
    f.write("\\begin{table}[h]\n")
    f.write("\\centering\n")
    f.write("\\caption{Resumen del Proceso de Limpieza de Datos (Versión Imputada)}\n")
    f.write("\\label{tab:cleaning_imputed}\n")
    f.write(summary_df.to_latex(index=False, escape=False))
    f.write("\\end{table}\n")

print("\nResumen guardado en: 02_output/tables/cleaning_summary_imputed.tex")
...
print("\n" + "="*80)
print("LIMPIEZA DE DATOS COMPLETADA EXITOSAMENTE (VERSIÓN IMPUTADA)")
print("="*80)

print("\nARCHIVOS GENERADOS:")
print("  1. 00_data/cleaned/data_cleaned_imputed.csv")
print("  2. 02_output/figures/missing_heatmap_imputed.png")
print("  3. 02_output/figures/outliers_income_boxplot_imputed.png")
print("  4. 02_output/tables/cleaning_summary_imputed.tex")

print("\nCOMPARACIÓN CON BASELINE:")
print("  Baseline: N=7,201 (drops missing income)")
print("  Imputed:  N=8,153 (imputes missing income)")
print(f"  Diferencia: {len(data_filtered) - 7201} observaciones")

print("\nPRÓXIMOS PASOS:")
print("  1. Ejecutar descriptivas versión imputada")
print("  2. Ejecutar regresiones versión imputada")
print("  3. Análisis de robustez (comparar baseline vs imputed)")
